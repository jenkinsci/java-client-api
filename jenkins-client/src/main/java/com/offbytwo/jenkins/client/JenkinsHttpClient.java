...
import com.offbytwo.jenkins.client.util.*;
...
public class JenkinsHttpClient implements JenkinsHttpConnection {
  ...
  public JenkinsHttpClient(URI uri, HttpClientBuilder builder) {
    this(uri, addConnectionConfig(builder).build());
  }
  ...
  protected static HttpClientBuilder addConnectionConfig(final HttpClientBuilder builder) {
   ...
   */
  @Override
  public <R extends BaseModel, D> R post(String path, D data, Class<R> cls, boolean crumbFlag)
      throws IOException {
  ...
  public <R extends BaseModel, D> R post(String path, D data, Class<R> cls,
      Map<String, File> fileParams, boolean crumbFlag) throws IOException {
   ...
   */
  @Override
  public void post_form(String path, Map<String, String> data, boolean crumbFlag)
      throws IOException {
    HttpPost request;
    if (data != null) {
      ...
      request = new HttpPost(
          UrlUtils.toNoApiUri(uri, context, path) + StringUtils.join(queryParams, "&"));
      request.setEntity(stringEntity);
    } else {
   ...
   */
  @Override
  public HttpResponse post_form_with_result(String path, List<NameValuePair> data,
  ...
  /**
   * Closes underlying resources. Any I/O errors whilst closing are logged with debug log level
   * Closed instances should no longer be used Closing an already closed instance has no side
   * effects
   */
  @Override
  public void close() {
